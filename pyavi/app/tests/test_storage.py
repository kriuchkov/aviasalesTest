...
def test_storage():
    ...
    s = storage.load_xml(XML)
    assert len(s) == 2

    output = storage.get_itinerary("DXB", "BKK", False)
    assert len(output) == 2

    # Самый дешовый маршрут
    h = []
    storage.optimal_itinerary(output, h, lambda i: i.price())
    ...
    h = []
    storage.optimal_itinerary(output, h, lambda i: i.duration())
    item = heapq.heappop(h)
    assert item[0] == 38100.0

    # Самый медленный маршрут
    h = []
    storage.optimal_itinerary(output, h, lambda i: -i.duration())
    item = heapq.heappop(h)
    assert item[0] == -47100.0
